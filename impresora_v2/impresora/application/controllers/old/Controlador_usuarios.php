...
//        var_dump($data['mensaje']); // mensaje
    ...
    public function add_user_invitado($id = null) {
        ...
        if ($id == null) {
            ...
            $data["pagina"] = "vista_respuesta";
            $this->load->view('conjunto_vistas', $data);
        }
//        var_dump($id);
//        var_dump($this->session);
//        $se = $this->session->set_userdata($id);
//        var_dump($se);
//        $this->load->view('panel/panel_regist ro'); // "V" carga todos los usuarios y texto en la vista
    }
    ...
    public function add_user() {
        ...
        //
        //
//        $insert = $this->modelo_usuarios->user_add($nombre, $apellidos, $password, $activo, $email, $tipo); // el insert no hace falta
//        if ($insert == TRUE) {
//            $this->panel("Usuario añadido con éxito");
//        }
    }

    /**
     * Cambiar a get_all_users($id)
     * 
     * Usa el parametro ($id) para seleccionar 
     * el valor que queremos usar para hacer 
     * el filtrado de datos de la bd
     * 
     * @param type $id
     */
...
////            $data['todos_usuarios'] = $this->modelo_usuarios->get_id_users($id); // funciona correctamente
