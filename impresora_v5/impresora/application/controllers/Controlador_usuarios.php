...
//Carga en 'Array' la 'Vista' - 'login/view_login' 
    ...
    public function check_login() {
        ...
        $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|min_length[13]|max_length[30]');
        $this->form_validation->set_rules('pass', 'Password', 'required|max_length[30]');
        //mensaje para las reglas    
        $this->form_validation->set_message('rule', 'Error Message');
        $this->form_validation->set_message("required", "El campo %s es obligatorio");
        $this->form_validation->set_message('email_check', 'Lo siento : Has introducido un correo invalido');
...
//                Redirige el flujo de ejecucion de la aplicacion
                $this->comprobar_usuario_activo($tipousr, $activo);
...
// Cargamos la vista        
    ...
    /**
     * Redirige al panel para invitado
     */
     ...
     * array (size=6)
      ...
      'nombre' => string 'us1' (length=3)
      'apellidos' => string 'us1' (length=3)
      'password' => string '1234' (length=4)
      'email' => string 'us1' (length=3)
...
// Redirige el flujo del programa al index con un mensaje            
    ...
    public function comprobar_usuario_activo($tipo = null, $activo = null) {
...
//        var_dump($activo);
        ...
        if ($tipo == 0 && $activo == 1) {
            $this->panel();
        } else if ($tipo == 1 && $activo == 1) {
            $this->panel_usuario();
        } else if ($tipo == 1 && $activo == 0) {
            $this->index($this->mensajes_activacion());
        } else if ($tipo == 0 && $activo == 0) {
            $this->index($this->mensajes_activacion());
        }
    }

    public function mensajes_activacion() {

        if ($this->session->tipousr == 0 && $this->session->activo == 0) {
//            Carga el mensaje
            $data['activar_admin'] = "Cuenta de Administrador sin Activar";
            $this->load->view('panel/panel_activar_admin', $data);
        } else if ($this->session->tipousr == 1 && $this->session->activo == 0) {
//            Carga el mensaje
            $data['activar_usuario'] = "Cuenta de Usuario sin Activar";
            $this->load->view('panel/panel_activar_usuario', $data);
        }
    }
